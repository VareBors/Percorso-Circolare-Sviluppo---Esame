...
let sectionIds : string[] = ['#HomeDiv','#ResourcesDiv','#LessonsDiv','#CoursesDiv','#RoomsDiv'];
//ActiveSection -> Variabile che indica quale sezione è visualizzata
let ActiveSection : string;
function ShowSection(arrayIndex : number){
...
function Search(){
    if(ActiveSection === "#HomeDiv") return;
...
function OpenResourceDetail(Id : number){
    ...
    .done(function (resource) {
        ...
        $('#saveResource').hide();
        ($('#resourceModal') as any).modal('show'); //Cast ad any per evitare errori di compilazione di typescript.
    });
}
function OpenLessonDetail(Id : number){
    ...
    .done(function( lesson ){
        ...
        $('#lessonEnd').val((lesson.endDate as string).substr(11,5));
        ...
        ($('#lessonModal')as any).modal('show');
    });
}
function OpenCourseDetail(Id : number){
    ...
    .done(function(course){
        ...
        ($('#courseModal') as any).modal('show');
    });
}
function OpenRoomDetail(Id : number){
...
function SaveResource(){
    ...
    if((resourceForm[0] as any).checkValidity()){
        var resource= {
        ...
        (resourceForm[0] as any).reportValidity();
    }
}
function SaveLesson(){
    var lessonForm = $('#lessonForm');

    if((lessonForm[0] as any).checkValidity()){
        var lesson =
        {
        ...
        lesson.endDate = (lesson.startDate as string).substring(0,11)+$('#lessonEnd').val();
        ...
        (lessonForm[0] as any).reportValidity();
    }
...
function SaveCourse(){
    ...
    if((courseForm[0] as any).checkValidity()){
        var course = {
        ...
        (courseForm[0] as any).reportValidity();
    }
}
function SaveRoom(){
    var roomForm = $('#roomForm');

    if((roomForm[0] as any).checkValidity()){
        var room = {
        ...
        (roomForm[0] as any).reportValidity();
    }
}
//Funzioni per la gestione delle select all'interno delle modali
function LoadLessonModalSelect(courseId : number = 0, teacherId : number = 0, roomId :number = 0, creatorId : number = 0, students : number[] = undefined){
...
function LoadCourseSelect(selectId : string, courseId : number = 0){
    $.getJSON('http://localhost:49267/api/Courses/GetAll')
    .done(function (courses) {
        $(selectId).empty();
...
function LoadResourcesSelect(selectId : string, useEmptyField : boolean, selectedRowId : number = 0,students : number[] = []){
    $.getJSON('http://localhost:49267/api/Resources/GetAll')
    .done(function (resources) {
        $(selectId).empty();
...
function LoadRoomsSelect(roomId : number = 0){
    $.getJSON('http://localhost:49267/api/Rooms/GetAll')
    .done(function (rooms) {
        $('#lessonRooms').empty();
...
function AddStudentToList(){
    if($(`#globalResources`).val() === undefined) return;
...
function RemoveStudentFromList(){
    if($(`#lessonStudents`).val() === undefined) return;
...
function SetCourseEndMin(){
    $('#courseEnd').attr('min',$('#courseStart').val() as string);
}
...
function RemoveElement(){
    var elementID : number = parseInt($('#removeElementId').val().toString());
    var url : string = `http://localhost:49267/api/${$('#removeElementType').val()}/Remove/${elementID}`;
